...
outputJsonFile = 'src/output/TMC/arraylist.json'
outputDirectory = 'src/output/TMC-Rename/json/'

FileLocations = [
    {
        "type": "Mint",
        "imageSourceDirectory": "src/output/TMC/images",
        "imageOutputDirectory": "src/output/TMC-Rename/images",
        "search": "src/output/TMC/json/*.json"
    }]
...
async def combineTypes(FileLocations):
    ...
    for genType in FileLocations:
        ...
        for currentItem in thisGlob:
            ...
            FullArray.append(
                {"id": currentID, "type": genType["type"], "path": currentItem, "image": genType["imageSourceDirectory"]})

    with open(outputJsonFile, 'w') as output_file:
    ...
    for currentFile in FullArray:
        ...
        with open(currentFile["path"], 'r') as infile:
            ...
            thisJson["image"] = mintImagePrefix + strNewID + ".png"
            ...
            with open(outputDirectory  + strNewID + '.json', 'w') as output_file:
    ...
    for currentFile in FullArray:
        ...
        await runCopy(currentFile["image"] + "/" + str(theID) + ".png", FileLocations[0]["imageOutputDirectory"] + "/" + str(newID) + ".png")
